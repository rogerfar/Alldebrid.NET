...
$binDirectoryPath = ".\AllDebridNET\bin"
if (Test-Path $binDirectoryPath) {
    Remove-Item -Path $binDirectoryPath -Recurse -Force
}

$csprojPath = Join-Path -Path $scriptPath -ChildPath "AllDebridNET\AllDebridNET.csproj"
...
if (Test-Path $csprojPath) {
    ...
    Write-Error "AllDebridNET.csproj file not found."
	exit
}

dotnet publish ".\AllDebridNET" -c Release
...
$packageFilePath = Get-ChildItem -Path ".\AllDebridNET\bin\Release" -Filter "*.nupkg" | Select-Object -First 1
